...
export class ScryScraperService {
  ...
  constructor(config: ScryScraperConfig) {
    this.s3Client =
      config.s3Client ??
      createS3Client({ bucketName: config.s3BucketName });
  ...
  private async fetchSetFromAPI(setCode: string): Promise<ScryfallSet> {
    ...
    try {
      ...
      if (!validation.success) {
        ...
        throw new Error(
          "Invalid Scryfall API response for set " + setCode + ": " + errorMessages
        );
      }
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 429) {
          ...
          throw new Error(
            "Scryfall API rate limit exceeded for set " + setCode
          );
        }
