...
async function npmPublish(pkg, tarFilePath, options, conf, otpCache) {
  ...
  const { scope } = (0, import_npm_package_arg6.default)(pkg.name);
  const opts = {
    log: npmlog_default,
    ...remainingOptions,
    projectScope: scope
  ...
  if (!dryRun) {
    ...
    const [tarData, npmCliPackageJson] = await Promise.all([
      ...
      await (0, import_package_json.prepare)(import_path13.default.dirname(manifestLocation))
    ]);
...
var import_package_json, import_fs_extra5, import_libnpmpublish, import_npm_package_arg6, import_path13;
...
var import_node_fs, import_is_stream, import_make_dir, import_path15, import_temp_dir, import_util, uuid, writeFileP, tempfile, writeStream, temp_write_default;
...
var import_make_fetch_happen2, import_path17, GitLabClient;
...
var import_npm_package_arg8, childProcess12;
...
var import_devkit6, import_fs5, import_path20, import_slash2, childProcess15, resolvedPrettier;
...
var import_devkit7, import_load_json_file3, import_path21;
    ...
    var PublishCommand = class extends Command {
      ...
      async execute() {
        ...
        if (this.options.summaryFile !== void 0) {
          ...
          output(message.join(import_node_os2.default.EOL));
        }
      ...
      async resetChanges() {
        ...
        const dirtyManifests = [this.project.manifest].concat(this.packagesToPublish).map((pkg) => import_node_path.default.relative(_workspaceRoot, pkg.manifestLocation));
        try {
      ...
      execScript(pkg, script) {
        const scriptLocation = import_node_path.default.join(pkg.location, "scripts", script);
        try {
        ...
        const mapper = (0, import_p_pipe.default)(
          ...[
            ...
            (pkg) => pulseTillDone(packDirectory(pkg, pkg.location, opts)).then((packed) => {
              tracker.verbose("packed", import_node_path.default.relative(this.project.rootPath, pkg.contents));
          ...
          ...[
            (pkg) => {
              ...
              return pulseTillDone(
                queue2 ? queue2.queue(() => npmPublish(pkg, pkg.packed.tarFilePath, pkgOpts, this.conf, this.otpCache)) : npmPublish(pkg, pkg.packed.tarFilePath, pkgOpts, this.conf, this.otpCache)
              ).then(() => {
      ...
      async copyAssets(pkg) {
        ...
        if ((0, import_node_path.normalize)(pkg.location) === (0, import_node_path.normalize)(pkg.contents)) {
          return;
        }
        const _workspaceRoot = process.env["NX_WORKSPACE_ROOT_PATH"] || import_devkit8.workspaceRoot;
        ...
        for (const asset of assets) {
          if (typeof asset === "string") {
            ...
            for (const file of files) {
              filesToCopy.push({
                from: (0, import_node_path.join)(pkg.location, file),
                to: (0, import_node_path.join)(pkg.contents, file)
              });
            }
            ...
            for (const file of files) {
              filesToCopy.push({
                from: (0, import_node_path.join)(pkg.location, file),
                to: (0, import_node_path.join)(pkg.contents, asset.to, (0, import_node_path.basename)(file))
              });
            }
          } else {
            throw new ValidationError(
